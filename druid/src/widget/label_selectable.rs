...
impl From<Selection> for std::ops::Range<usize> {
...
impl<T: TextStorage> SelectableLabel<T> {
    /// Create a new [`RawLabel`].
    ///
...
impl<T: Data> Widget<T> for SelectableLabel<T> {
...
impl<T> Deref for SelectableLabel<T> {
...
impl<T> DerefMut for SelectableLabel<T> {
